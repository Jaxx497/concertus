...
const SIDEBAR_PADDING: Padding = Padding {
    left: 3,
    right: 4,
    top: 2,
    bottom: 1,
        ...
        *display_state.offset_mut() = current_offset;
        ...
        let list = List::new(list_items)
            .block(
                Block::new()
                    .bg(theme.bg)
                    .title(Line::from(pane_title).left_aligned().fg(theme.text_focused))
                    .title(
            ...
            .highlight_style(Style::new().fg(theme.text_highlighted).italic())
            .scroll_padding(4);
