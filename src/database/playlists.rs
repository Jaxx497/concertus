...
use crate::{
    database::queries::{
        ...
        PLAYLIST_BUILDER, UPDATE_PLAYLIST,
    },
    domain::Playlist,
    Database,
};
...
impl Database {
    ...
    pub fn build_playlists(&mut self) -> Result<IndexMap<(i64, String), Vec<u64>>> {
        ...
        let rows = stmt.query_map([], |r| {
            ...
            Ok((playlist_id, song_id, name))
        })?;

        let mut playlist_map: IndexMap<(i64, String), Vec<u64>> = IndexMap::new();

        for row in rows {
            let (playlist_id, song_id_opt, name) = row?;
